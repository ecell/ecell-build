...
FOR /F "usebackq" %%A IN (`""%PYTHONHOME%\python" -c import" "sys;import" "os;sys.path.append(os.environ['STAGE']+'\lib\site-packages');import" "ecell.config;print" "ecell.config.per_app_dir_name"`) DO SET PKGDIR=%%A

iscc /dPlatform=%PLATFORM% /dPythonVersion=2.5 /dNumpyVersion=1.0.1 /dGtkVersion=2.6 /dVersion=%ECELL_VERSION_NUMBER% /dPkgDir=%PKGDIR% /dGeneratedSampleEmlDir=%GENERATED_SAMPLE_EML_DIR% ecell3-installer.iss
